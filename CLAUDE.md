...
2. **Consolidation** — Beacon → an Echo assay type; one plate engine; Helix + Plasmids;
   Blot + Blueprint's Gel Designer; Cell Archive + Incubator + Iceberg into one "Cells"
   lifecycle. **Lumina is not on this list** — see *Lumina is Echo's counterpart*.
...
before this change are overwritten. The flag is at **`_presetV17`** as of 2026-09-25 (the NanoBRET 6-well seed's `fmtOwn`); **bump it if you edit `PRESET_SEED` or `EXTRA_PRESET_SEED` again.**
...
**Phase 2 — Beacon → an Echo assay type — is deliberately not in this pass.** Auditing before consolidating is the whole reason the audit came first: two of the three
...
## Lumina is Echo's counterpart, not a mode of it (corrected 2026-09-22)

This file said three times that Phase 2 folds **Lumina → an Echo mode**. Wrong, and Jon said so
plainly: *"lumina no es un modo de echo, es su contrapartida. es para cuando haces ensayos a
mano y no con echo, por lo que tú le dices qué compuestos son y en qué set up."*

The two are the same analysis reached from opposite ends. **Echo starts from the machine**: a
Labcyte picklist says what went into every well, and the plate map is derived from it. **Lumina
starts from you**: there is no picklist because you pipetted it, so you declare the layout —
compound, start well, direction, dilution factor, replicates — and the reader file drops
signal onto that grid. Folding one into the other would mean an Echo whose primary input is
absent, which is not a mode; it is a different app with the same fitter. The shared 4PL engine
is already handled the right way (`tools/sync_fit_engine.py`, Echo canonical).

**Most of it already exists.** Lumina today has the plate-format grid, *Add compound (serial
dilution)* with start well · direction · dilution factor · replicates, the four assay types
(HiBiT · CTG · Displacement · Gain of signal), XLS/XLSX reader drop straight onto the grid,
and the fitter in sync with Echo's. What it does **not** have, measured against Echo:

- **No `sendResultsToLabbook`, no `dhub:context`** (0 occurrences of each). Echo's potencies
  reach the experiment record, the Results tab and the Methods paragraph; Lumina's stop at the
  screen. A manual assay is exactly as publishable as an Echo one, so this is the real gap.
- **No out-of-range EC50 flag.** The 2026-09-04 Echo audit added `EC50<range` / `EC50>range` —
  a midpoint outside the doses tested fits its own half at R² 0.997 and reaches paper looking
...
### Lumina, simplified around the plate (2026-09-25, fourth pass)

Jon liked the plate and compound workflow and asked for its principles — not its widgets — across
the rest of Lumina, with every control that does not earn its place removed. The audit found the
plate was the only part built that way; around it sat a Setup card left over from a dialog, three
output tabs, and toggles for things nobody decides.

**Two tabs: Plate · Results.** Input · Visualization · Results became one screen for making the
plate and one for reading it.

- **The Setup card is gone** (Files · Layout · Analysis · Output, with a hidden ✕ and Done). Assay
  type and 96/384 are the head of the plate card. The **Reader format** menu went: it offered
  PHERAstar and GloMax, and GloMax only refused the file — a question with one answer. The parser
  finds the lettered grid itself and now says when it fell back to reading by position; a GloMax
  parser goes in `handleReaderFile`, picked from the file, when an export turns up.
  `parseGloMaxSheet` (an AUDIT-KEEP placeholder) is deleted.
- **Five steps replace the three and the Layout / Signal / Compounds subtabs:** Gradient ·
  Compounds · Controls · Names · Readings. Each says what is done; clicking one goes there (the
  layout steps select their wells, Names is the list, Readings is the plate with its values).
  Controls is a step because it was only reachable through a mode button and a legend chip.
- **The panel sits beside the plate and shows nothing until wells are selected.** Idle, it says
  what to do next (`_lmIdleHtml`, from `_lmNextStep`) plus the shortcuts and, on an empty plate,
  *Load an example plate*. With a selection it infers what the wells are for (`_lmMode`: all
  controls → Control; all on a gradient → Compounds; off the gradient when compounds exist and
  controls do not → Control; else Gradient) and shows that one form. The switch overrides it for
  that selection only — `LM_MODE` resets on every new selection. *Replicates are Rows/Columns* is
  shown only for wells with no gradient under them: on a gradient the replicates run across it,
  and the other answer was always wrong. *Clear* appears only when the wells hold something.
- **The legend under the plate is gone**: a compound's label on the plate is the handle for its
  wells (`.w-zone span[data-z]`; `_lmHit` reads through labels with `elementsFromPoint`, so a drag
  does not stall on one), and double-clicking a control well takes all the controls.
- **Reader files drop anywhere on the card** (`lm-veil`); the Readings view says where each
  plate's numbers came from, with *Replace*, or is itself the drop target. Its wells are coloured
  by the reading — which is what the Visualization tab's plate heatmap was for, so that canvas
  went. **Fit curves** is disabled with the reason beside it until there is something to fit;
  Undo and Clear layout appear only when there is something to undo or clear.

**Results never go stale.** `_lmFitSig()` fingerprints the assay, format, layout, readings,
names, controls and fit settings; opening Results refits when it differs (`_lmEnsureFit`), and a
changed fit setting refits on the spot. `runLuminaAnalysis(opts)` returns true/false and no
longer navigates. The table is the sample list — a row draws its curve beside it, over every
other fit in grey. That overlay replaces the LogEC50-vs-Hill scatter; the Samples chip list went
with it. **Fit settings** live on Results as sentences, hide what the assay ignores (instead of
greying it) and hide the bottom floor when the bottom is fixed; a dot on the button says when
anything differs from the defaults. **The ten Output-column toggles are gone**: the export always
carries every column (a spreadsheet hides a column in one click), the screen shows what a potency
is read by — sample, potency, 95% CI, Dmax/Span/Emax, Hill, R², and a QC chip only when flagged.
Export is one menu (Excel · Copy table · Raw CSV). **Send to Labbook exists only inside dHUB**;
standalone, Export is the primary action.

Supersedes the *eighteen suggestions* note that Lumina's setup is "inline beside the plate" with
`closeSetupModal` a no-op — neither exists now.
...
proteínas) y no target engagement… el usuario hace drop del excel de PHERAstar, marca los
controles necesarios en la placa… con esto beacon hace los ratios (en mBRET) de toda la placa
y en visualizaciones se ve la tag de controles."* So the default flow for a **gain-of-signal
(PPI)** run is three things, and deliberately nothing more — the layout changes with every
assay, so which compound sits where is the *next* thing the plate map is asked to carry.

- **The five NanoBRET controls are plate-map roles** — no HaloTag ligand · donor only ·
  acceptor only · no substrate · mock transfection — painted by click or drag exactly as
  background and DMSO already were. They are flat well lists, not dose groups: a control is a
  set of wells with no concentration and no colour sequence, so `pmIsFlatRole()` is what every
  paint route now asks instead of `role==='background'||role==='dmso'`. `BEACON_CONTROLS` is
  one declared list, so a sixth control is a data entry, not three more branches.
- **Plate tab — mBRET for every well.** `mBU = (acceptor/donor) × 1000`. **Corrected** mBRET
  subtracts the mean of the no-HaloTag-ligand wells: that control is the same transfection with
  the ligand left out, so what it reads is donor bleed-through into the acceptor channel, and
  every well carries it. With no such wells painted there is nothing to subtract, so the view
  **says so and shows raw** rather than inventing a zero. A well whose donor or acceptor is
  missing or zero is `null`, never 0 — a donor of zero is "not read", not "no signal".
- **Controls tab — the bar graph.** Donor beside acceptor for each control that has wells,
  raw counts on one shared axis from zero, with SD whiskers and the well count. Raw counts and
  not the ratio on purpose: what you check here is whether the donor fired at all (no
  substrate, mock) and whether the acceptor channel is clean, and a ratio hides both. A table
  under it carries the same numbers plus each control's raw mBRET.

`exportPlateMBretXLSX` writes well · donor · acceptor · raw mBRET · corrected · which control.
Found by the audit: a control well carries a fixed pastel, so in dark mode the inherited light
ink sat on it at **contrast 1.23** — the same trap as Labbook's plate numbers. The ink on a
fixed colour is fixed too.
...
## The Experiment Designer — a plan built out of modules (2026-09-22)

Jon's brief: creating an experiment should be a wizard, not a form. Ask what kind of experiment
it is, ask the parameters that kind has, let the user pick the **modules** it is made of, walk
them one at a time, and — the part that matters — keep the modules aware of each other, so a step
knows what the steps above it already put in the well, what day it falls on and what plate it is
in, without any of it being typed twice. Then let the whole thing be saved as a reusable design.

**Most of the machinery already existed and was not joined up.** The step library is a catalogue
of procedures with live calculators; `applySetupToBlocks` pushes the setup's answers into every
block; `syncChainedInputs` + `VOL_ADD` chain the well volume down the run. What there was no
route to was *composing* an experiment out of those parts — a preset's shape could only be
changed by editing `labbook.html` or by adding steps to a run after it had been created.

### The designer builds a preset and hands it to the one creation path

`dsTemplate(DS)` is preset-shaped — `{name, baseType, title, setup, blocks}` — and goes through
**`buildExperimentFrom`**, which is `createExperiment`'s body extracted. The New-experiment
window is now a reader of its own form that calls the same function. So there is one place where
the code, the prefix, the setup, the plate, the Echo picklist and the protocol steps are decided,
and nothing for the two routes to disagree about. A design started from a preset keeps its
`presetKey`; one built from scratch carries an **empty** one, because *"update the wording from
the preset"* is not an offer that means anything for an experiment that was not copied from one.

**`dsPseudo(D)` is the preview, and it is not a second implementation.** It instantiates the
draft exactly as `buildExperimentFrom` does, runs `applySetupToBlocks` and then
`syncChainedInputs` over it, and everything on screen — the day plan, the state strip above each
module, the live recipe, "what is in the well" — is read off *that*. If the preview and the
created experiment ever disagree, one of those three has been bypassed. Verified block for block:
same titles, same day offsets, same dates, same chained volumes.

### A module carries a gap, never a day

`m.gap` is *how many days after the previous module*; the absolute day is `dsDays(D)`, derived on
every render. That is the whole reason a reorder can recompute the calendar: an absolute day is a
fact about the order the module used to be in. Verified — dropping a media change into the middle
pushes everything after it by a day and each module keeps its own interval; moving the readout
above the compound addition re-dates both **and** re-derives the spike's starting volume from 90
to 140 µL, so the 10× spike goes from 10 µL to 15.56 µL with nothing retyped.

**A field the setup owns is shown, not offered.** `applySetupToBlocks` writes `format`,
`cellLine` and `cellsPerWell` over whatever a block carries, so offering them as editable boxes
in the designer would be exactly the defect the preset editor's own comment records — *"changing
the calculator looked like it worked and never did"*. They render disabled, labelled *from the
plate format* with a link back to step 2, and they show the **effective** value (what the setup
is about to write), not the module's own: reading the module's showed "384-well" under a state
strip that said 96. A chained field is the same idea in the other direction — it shows what the
steps above leave in the well, marked *from the steps above*.

### A question that only arises because of another answer

`setupFieldsFor(presetKey, su)` filters on a field's `when(su)`. Asked with no answers — by
`setupDefaultsFor`, by the Methods walk, by the preset editor's first paint — **every** field
comes back, so a conditional field still has a default and an experiment created before the
condition existed still resolves. CTG leads with *Dose–response series*; the compounds, the top
concentration, the dilution factor, the points and the replicates hang off it, and with it off
they are not asked and nothing about a series reaches the Methods. `_pubSetupText` skips a field
that was never asked, and a `check` never printed there anyway.

### Modules are the step library, shelved

Eighteen entries joined `STEP_LIBRARY` (media change, wash, incubation, harvest, replate, the
three NanoBRET calculators, substrate + read, plate read, sample prep, gel, antibodies, imaging,
PCR, assembly, transformation, a free step) — all of them prose that already existed inside one
preset or another with no way to reach it except by creating that whole preset. `MOD_CATS` is the
order and the shelf; **a library entry named in no category still appears, under General** —
a module you can add but cannot find is not a module. The step palette gained all eighteen too.
...
**Three doors, and they are different questions.** The `+ Add` menu asks how you want to build
it: *design it step by step* · *start from a design* · *quick create from a preset*. The quick
window stays exactly as it was — it is the two-click route for a run you do every week — and
`dsFromQuick()` hands its preset, setup, date, title and tag over to the designer when it turns
out the run is not routine after all, rather than starting again.

### Three defects the designer surfaced, all of them older than it

- **A setup hook that GENERATES a block wrote it in the wrong shape.** CTG makes one readout per
  timepoint; handed instantiated blocks it went on returning preset-shaped ones. Any readout hour
  the preset has no block of its own for — anything but 24 or 72 — reached the experiment with
  **no id, no date and no dayOffset**: unfindable by `_blkById`, so it could not be ticked, noted
  or scrolled to; sorted to the front of every run order by an empty date; and drawn on day 0
  whatever hour you asked for. `_normBlocks` normalises after the hook rather than inside it, so
  the next hook that generates a block cannot repeat it. Reproduced against the committed build
  before it was touched.
- **Creation never ran the chain.** `syncChainedInputs` fired on insert, drag, delete and the
  first `renderExpEditor` — so a record was *created* carrying the preset's own well volume, a
  number about somebody else's experiment, and the Methods paragraph was built from it. One line
  in `finish()`.
- **And the chain read as a deviation.** `applySetupToBlocks` stamps `calcSeed` after the setup,
  for the stated reason that answering the setup is planning and not deviating — the chain is the
  same thing and was not doing it. Every NanoBRET biosensor experiment was born reporting *"Already
  in the well (µL) 111.1 → 111"*: a deviation banner on a run where nothing had happened. A value
  you typed is still skipped, so a real override still shows.

### The harness passed by accident

Removing that banner made the day content 78 px shorter, and `tools/mobile_sweep.mjs`'s long-press
test began to fail — on a build where nothing about long press had changed. Its press point was
derived from wherever the row happened to sit and clamped toward the middle of the viewport, so on
a page whose steps start below the fold it landed **off-screen**, and the `|| row` fallback then
dispatched on the row anyway: the test had been passing without ever pressing anything. It scrolls
the row into view and presses its middle now, and says so when the point is covered. Both builds
pass it.

**Verified**: `check_css` / `check_js` / `audit_app --xref` / `check_shared` clean; the runtime and
alignment audits clean over 17 designer screens at 1440 / 1180 / 1024 / 900 / 760 / 600 / 375 in
both themes with no horizontal overflow; `tools/mobile_sweep.mjs` at **0** on Chromium, WebKit and
embedded with seven designer screens added to it; the designed experiment created in the source
file, `labbook-standalone` and the Labbook PWA build, matching its preview block for block; the
quick window, its Echo-picklist branch and the handover re-tested after the refactor; the five
profiles rebuilt.

Three alignment findings were mine and are fixed at the rule rather than the instance: the module
row was the `margin-left:auto` idiom for the sixth time (the spacer takes a whole line at phone
width, so the buttons start at the same left edge as the name); a derived-value note placed *after*
an input takes part in `.ci-f`'s space-between and lifts the control 19 px off the line its
neighbours sit on, so it belongs with the label; and a checkbox whose label is a sentence needs the
whole row in a panel this narrow, not the two columns `.ci-f.chk` already asks for.

### Driven through a real run in the Hub (2026-09-23)

Building a HiBiT BRD4 degradation from modules inside dHUB, rather than from the outside,
turned up nine more. Seven were the designer's; two were older and it only made them visible.

- **"Designer" was in the rail and in Labbook's column, both lit.** Inside dHUB the rail names
  the **workspace** and the column names the **surface** within it — the Designer is a peer of
  Journal and Visualize, not a Planner surface, so `lbWorkspace()` answers `designer` for it and
  the column lists it only where there is no rail to own it. That also removed the special case
  `lb:go planner` had needed to avoid landing on it.
- **The rail went stale whenever Labbook navigated itself.** Create an experiment from the
  Designer and the screen is the Planner while the rail still says Designer — and the same was
  already true of following a link out of the Journal. The shell's section is set by the button
  you pressed, and Labbook is the only thing that knows where it actually went, so it says:
  `{type:'lb:ws'}` on every workspace change, and the shell lights the entry that matches.
- **Reusing a class name is not reusing its rule.** `.nm-pv-*` is scoped to `.nm-preview` and
  `.nm-lbl-sub` to `.modal.nm`, so the review's day plan rendered as unstyled spans running into
  each other — *"Day 0 · 22/09/2026 TueCell seeding"* — and every sub-label came out at full
  weight. Both are **rescoped**, not duplicated; the call `PRINT_CSS` made for the copy stage.
- **The prose showed its tokens.** Every other surface renders a step's `{{c.key}}` as the
  calculator's value in a non-editable span and writes it back as the token before storing. The
  designer showed `{{c.bufferUL}} buffer + {{c.lgbitUL}} LgBiT` to the one person reading the
  step to decide whether it is right. `fillCalc` in, `_unfillCalc` out — the stored html still
  carries the token, which every "is this still the preset's text" proof depends on.
- **Asking for 4 plates made 8.** `applySetupToBlocks` scales each block's plate count by its
  ratio to the seeding block's — right for a preset whose blocks were authored together (D2B
  really does lyse two plates per assay plate), an artefact when the modules come from the
  library, where seeding defaults to 1 plate and CellTiter-Glo to 2. A library module carries
  no ratio.
- **"What is in the well" listed steps that put nothing in it** — *"Results / data analysis —
  45 µL"*, which is the row above's number beside a step that is not a bench step at all. Only a
  step that adds volume is listed, and only such a step prints one on its row.
- Plus: the aim's placeholder was cut in a third of a row (it takes the whole one); the footer
  echoed a code the review already shows in a field two inches above; *"Starts / days after X"*
  reads *"Days after X"*; and a calculator tag that only repeats the step's name is dropped —
  "Cell seeding · Cell seeding" is the same fact twice.

**Two that were older than the designer.** `.exh-code` was a fixed `width:15ch`, so every code
with a project prefix and a target tag — `DD_HB20260922_BRD4`, 18 characters — was shown as
`DD_HB20260922_B`, silently eating the part that says what the run was about. And **a library
step with no Methods sentence publishes its bench text**: `CALC_PUB` covers eight calculator
kinds and nothing else, so an Echo compound addition published *"Load the picklist
(&lt;code&gt;_Picklist.csv), Run the Echo and Attach the run's CSV to this experiment on the
Files tab."* — an imperative, a placeholder leaking as literal text, and an instruction about
where to file a file, in a Methods section. Seven library steps gained a sentence and the
results step `noPub`, which `pubSentence` already reads; `noPub` now survives all three routes a
library step reaches a block by.

### A box you type in and a box that is read have to be the same box (2026-09-23)

Jon, from the Designer: set 20,000 cells/well and 90 µL and the recipe said **1.22 M/mL and
110,000 cells/well** — it should be 0.22. Then, on the next module: *"en parameters ya he puesto
qué plásmidos quiero y qué relación de NanoLuc a HaloTag, y aquí aparece un plásmido random que
yo no he usado."*

**One root, and it is the read/write frame.** A library module lands with its own defaults —
3,000 cells in 30 µL of a 384-well — and `applySetupToBlocks` rescales them onto the design's
plate when the experiment is built. The configure screen (correctly, since 2026-09-23) shows the
**effective** value, the rescaled one; its `oninput` wrote back the module's **raw** one. So
20,000 typed into a 96-well design was stored as if it meant 384-well and read back as
20,000 × (0.32/0.056) = **110,000**. Reproduced against the committed build, digit for digit,
including the 11.62 M cells and the "4× the usual seed" warning the app then raised about a
number the user never entered.

`dsSyncMod(m, su, fresh)` moves the module onto the design's plate **once**, so the rescale in
`applySetupToBlocks` is afterwards a no-op and what you type is what is used. It is idempotent —
`rescaleSeedInputs` returns early once the formats match — so `dsDraw` calls it on every render
and a stale frame is impossible. Verified: 96→384→96 returns exactly 20,000/90, and the preview
and the created experiment agree input for input.

Two more boxes were in the same state and are now **owned** (shown, not offered): **# plates**,
which `applySetupToBlocks` recomputes from its ratio to the seeding block's so a number typed
there was overwritten the moment it was read; and `rtxmix`'s **plasmid**, which now carries the
setup's donors and acceptors. A freshly added module also takes its **# wells** from the plate —
but only when fresh, because 84 of 96 is a number somebody chose and overwriting it would be
this same defect in the other direction.

**And a calculator that cannot say what the parameters say now says so.** Feeding `rtxmix` the
plasmid names is half an answer: one master mix gives every well the same DNA, so a donor:acceptor
ratio has nowhere to go in it and the wells the ratio exists for — donor only, acceptor only,
mock — get no mix at all. `dsCalcMismatch` states that in the panel and offers
`lib:nbtx`, which makes one mix per condition and reads the plasmids, the ratios and the control
wells straight from the parameters. `dsUseMod(i,libId)` swaps the **module**, not just the
calculator: prose quoting `{{c.*}}` keys the new calculator does not have would be the next
silent wrong number.

### Seven more things the Designer lost (2026-09-23, same day)

Jon: *"sigue revisando el designer, a ver qué más se pierde."* Each one reproduced against the
committed build before it was touched, and each one is the same shape as the cells/well bug — a
value the wizard shows or accepts is not the value the experiment gets.

- **A chained input typed here was thrown away.** 250 µL into *Already in the well* → 90 µL used,
  silently. In a running experiment `calcUpd` marks it `b.calc.own[key]` and the field wears
  *yours · re-link*; the designer had the "from the steps above" label and none of the contract
  behind it. It has both now, and the number rides through `dsTemplate` into the record.
- **A wait typed here was thrown away**, and the box lied about it before you typed. A setup field
  marked `wait` overwrites `waitMin` wherever the module's prose quotes its token, so a D2B step
  showed the module's own wait while the run waited `{{treatH}}`. `dsSetWait` marks `waitOwn` —
  the rule `applySetupToBlocks` already followed — and both the box and the module row show the
  wait the step will really have.
- **Changing the type threw away every parameter already answered** and then said *"the modules
  you had are kept"*, which points at the half that survived. Most types ask several of the same
  questions in the same words. `setupTouched` records which answers were actually *given* — a
  value that merely equals the old type's default must not travel, or it silently overwrites the
  new type's own — and the toast now says how many carried over.
- **Saving a curated preset as a design could not reproduce it.** `dsTemplate` emitted no
  `layout`, no `setupAdd`, no `setupHide`, so *Save as a design* on the SPARK screen dropped the
  plate map that **is** the experiment, dropped the four questions the layout draws its geometry
  from (top µM, dilution factor, points, replicates) and brought back three questions the variant
  has no use for. All three ride on `DS` now and into the template.
- **Deleting a module re-dated every module after it.** A gap is "days after the previous step",
  so deleting a 2-day incubation moved the readout from day 3 to day 1. The gap is handed to the
  module that inherits its place: the days of the steps you are keeping are not the deleted one's
  to change, and pulling the schedule in is then one visible number.
- **The step prose showed raw `{{dmsoMax}}`** beside calculator values that rendered properly —
  the one box where you read the sentence to decide whether it is right was the one not showing
  what it would say. `_fillSetupSpans`/`_unfillSetup` give a `{{setupField}}` the same treatment
  `fillCalc` gives `{{c.*}}`, so the stored html still carries the token and every "is this still
  the preset's text" proof works unchanged.
- **Removing a calculator left the sentence quoting it.** The step reads "dilute the suspension to
  — for —", and where the calculator spoke for the whole block that is also what gets published.
  It says so before, not after. Also: the designer's number inputs had no `min="0"`, which
  `calcInputsHtml` has carried since a minus sign propagated into every recipe.

And the quick window's answers now arrive marked as given (`dsFromQuick` infers it from what
differs from the preset's defaults), or changing the type after handing over would drop them all.

### The moat was missing from the screen that composes an experiment (2026-09-24)

Third pass over the Designer, and the two findings are the biggest of the three rounds.

**`dsCreate` passed `protos:[]`, hard-coded.** The 34 Archive protocols with their live
calculators — what this file calls the moat, and the reason a stage is a dated block at all —
were unreachable from the wizard whose whole job is to assemble an experiment out of parts. Worse,
`dsFromQuick` dropped whatever the quick window had already been told: pick Gibson and
Transformation, decide half-way through that the run needs designing, and they were gone with
nothing said. The Echo picklist went the same way, and with it the plate map, the compounds and
the run's metadata. `D.protos` / `D.echo` now ride on the draft, a protocol strip sits under the
modules in step 3 (stage count, delivery variant, start day, remove), and the review's day plan
lists every stage on the day it lands on, exactly as the quick window's preview does.

Protocols are deliberately **per run, not part of a design** — a design stores its modules and
its parameters, not the protocols one run of it happened to follow — and the strip says so rather
than leaving it to be discovered.

**And your lab's day cadence for a protocol never reached any experiment.** `LB.data.protoDays`
is the whole of *"Protocol content is Archive's; cadence is yours… the layout converges over the
first few runs"*. It was read by `nmLoadStages` and `nmPreview` — the preview — and by **nothing
else**: `buildExperimentFrom` dated each stage from `st.day`, straight out of Archive. So you set
Gibson to run over four days, the preview agreed, and all four stages were created on day 0.
Archive states a day for 3 of 153 stages, so for essentially every protocol in the library the
feature was a picture. This is the **quick window too**, not only the Designer — it predates both
and was confirmed against the committed build before anything was touched. Verified after: the
plan and the record now agree day for day, from both creation paths.

`_protoSetStageDay` / `_protoResetDays` are one implementation called by both, because a cadence
is a property of the protocol and not of the window you happened to set it from.

### The plasmids you named, and a plate that is shown rather than promised (2026-09-24)

Fourth pass. Most of what was checked came back clean and is worth recording as checked:
duplicating a curated design and editing one both keep `layout`, `setupAdd` and `setupHide`
(`seedPresets` writes all three onto the stored preset, so the deep copy carries them); no preset
or library step has a `{{placeholder}}` in its *title*, which is the one string the designer still
shows raw; and the plate a designed run gets is correct — it was simply never shown.

**`buildExperimentFrom` wrote `plasmids:''`, always.** The cell line has come from the setup since
the setup existed; the plasmids never did, although a NanoBRET run names two of them in the very
first panel. So the experiment header said nothing, the report listed nothing, Cmd+K could not
find the run by its construct, and *What you need before you start* — whose whole job is to say
where a thing physically is — had no plasmid to look up unless the run happened to carry an
`nbtx` calculator. `setupPlasmids(su)` **splits** the comma-separated donor and acceptor lists
rather than pasting them in whole, because all three readers treat `e.plasmids` as a list; the
designer's own `_dsPlasmidText` is now that function joined with a `+`, since one mix holding
several plasmids is spoken differently from a list of preps to fetch. This is the quick window
too.

**The review asserted "with a plate map" and showed none.** The plate is the part of a run people
hand-draw, so it is the last thing that should be a promise. `experimentPlateFor(key,type,setup,e)`
is `buildExperimentFrom`'s own plate logic extracted, and the review draws the map from it — one
implementation, so the preview and the record cannot disagree. Verified signature-for-signature on
both branches: a curated layout (the SPARK screen, 12 compound blocks with their concentrations,
the controls and the legend) and a NanoBRET drawing its own from the conditions.

### Two of these were mine, from yesterday (2026-09-24, later)

Fifth pass, and the first two findings are in the code that fixed the third pass. Worth recording
as its own lesson: **a preview extracted from the creation path only stops lying if it takes every
branch of it.**

- **The review drew a blank plate for a run created from an Echo picklist.** `experimentPlateFor`
  was `buildExperimentFrom`'s plate logic extracted — except the picklist branch, which sits above
  it in an `if/else` and was left behind. So a run handed over from the quick window with a
  picklist previewed an empty 96-well and was created with twelve dosed ones. The branch is inside
  the function now, both callers go through it, and the label says *read from the Echo picklist*.
  Verified signature-for-signature.
- **The review ignored *Skip weekends* for a protocol's stages.** `_skipWeekends` accumulates a
  shift over the whole sorted list, so stages worked out separately can never match what
  `buildExperimentFrom` produces — it shifts modules and stages together. The protocol stages are
  built into `dsPseudo` now, before the shift, marked `dsProto` so the module readers ignore them.
  Gibson starting on a Thursday: the review said Sat 26 and Sun 27 while the record said Mon 28 and
  Tue 29; both now say Mon and Tue.

And one older: **`dsCreate` passed `blank:false`, hard-coded**, and `dsTemplate` emitted nothing —
so a Blank experiment opened in the designer came out as an ordinary one, and saving it as a design
lost what it was for good. `e.blank` is what folds the type/status/date grid behind a Details
button; it rides on the draft now.

Checked and clean: no preset block carries a per-block plate; `ctxDesign` offers the same four
verbs as the card; every key on `DS` is consumed by `dsTemplate` or `dsCreate`. One defensive
...
encontrarlos todos a la vez?"* The honest answer was that each pass explored rather than
inventoried, and that a fix can carry its own bug (two of the fourth Designer pass's were in the
third pass's code). The answer to that is not a more careful hunt but **`tools/invariants.mjs`**:
Playwright loads Labbook and checks, over **every preset and every library module**, the classes
of bug already found by hand — so a class once found cannot come back, in new code or in a fix.

| | invariant | the bug it was |
|---|---|---|
| I1 | the Designer's review = the record, step, date and plate | the Echo plate, weekends with protocol stages |
| I2 | quick window = Designer, same preset and date | `blank:false` |
| I3 | quick → "design it" carries everything | `protos:[]`, the picklist |
| I4 | a value typed in the Designer is the value stored | 20,000 → 110,000 cells, the chained volume, the wait |
| I5 | prose rendered for editing and written back is unchanged | `{{c.*}}` / `{{setup}}` spans |
| I6 | every draft key and every `buildExperimentFrom` argument is read | dropped keys |
| I7 | save as a design → create from the copy = same experiment | `layout`, `setupAdd`, `setupHide` |
| I8 | what the parameters name reaches the record's own fields | `plasmids:''` |
| I9 | the quick window's promised dates are the created ones | — |

It runs against the source and against `labbook-standalone.html` (the only build that embeds
Archive, so the only one where the protocol cases run), and **CI runs it before deploying** —
a finding stops Pages from updating. `node tools/invariants.mjs [--only=I1,I4] [--verbose]`.

**The harness was proven against the bugs it exists for**: each of eleven historical defects was
put back in a temporary copy and had to be caught. The first version caught four. Two lessons from
the seven it missed, and both are rules for writing the next invariant:

- **Relative checks cannot see a bug both routes share.** `plasmids:''` made quick and Designer
  agree perfectly. I8 is absolute — the plasmid the setup names must be in the record.
- **A harness that does the app's work for it cannot notice the app stop doing it.** It called
  `dsSyncMod` itself, and exercised modules alone, where there is no chain above and no plate
  that moves. Modules now enter by the Add button's own line, the plate is changed through the
  parameters form, text goes back through `dsSetHtml`, and every preset's own modules are typed
  into as well as the library's.

A crash inside the suite is reported as a finding, never as silence — the first mutation run
hid one behind a `grep ✗`.

**What it found on its first run, in code that had been "verified":**

- **Protocols + skip weekends moved the modules twice.** `buildExperimentFrom` shifted the
  modules, then shifted modules *and* stages again in `finish()`; `_skipWeekends` accumulates, so
  the record landed two days after the plan. Yesterday's fix made the *preview* right and left
  the record wrong. The shift is made once now.
- **A CTG readout configured in the Designer was thrown away.** The CTG hook generates one
  readout per hour and discarded any readout with no hour in its title — the one you added — so
  157 µL/well came out as 50 and a 2 h 15 wait as 40 min. An hourless readout is now the
  template: the generated ones keep its numbers and its wait; the wording stays hour-specific.
- **The SPARK review drew its plate without condition numbers**; the record was numbered by its
  first render. `experimentPlateFor` numbers a NanoBRET plate itself, so both are numbered once.
- The review showed no wait for a protocol stage that ends in one; and a Blank page became
  `<p></p>` through the Designer, which is not `:empty`, so its placeholder never showed.

**When a new bug is found, the fix is not finished until an invariant would have caught it** —
and, where the class is new, the invariant is added and proven by putting the bug back.

## The Report and exports under the invariants, a tour, and tabs that stay put (2026-09-24)

**R1–R7 join `tools/invariants.mjs`.** One experiment per preset is created and *dressed* with
content no seed has, each piece carrying a marker: a ticked step with a log, a result table with
a flagged row, an excluded row and a compound named `INV "Q", 2` (a comma and quotes), an
observation, an outcome, a file, a plate well, a changed calculator number. Then every way out
is asked whether it says so:

| | invariant |
|---|---|
| R1 | the Report carries every section's content, follows a change with nothing pressed, each section toggle removes what it names, hand-written wording is kept and goes stale when the record moves |
| R2 | no `{{token}}`, `undefined`, `NaN`, `Infinity` or double-escaped entity in the Report, Methods sheet, record PDF, bench sheet or What you need; the JSON parses |
| R3 | every export runs through its own function without an error and names the experiment; every button on the Report calls a function that exists |
| R4 | every step and every marker is in the record PDF, and each box in the export dialog removes what it names |
| R5 | JSON out → import is the same experiment and prints the same; a folder bundle carries every experiment in it |
| R6 | the results and steps CSVs parse back to the record's rows, commas and quotes included |
| R7 | a folder / project / all PDF carries every experiment in it; a Journal day, month and the whole Journal carry each step done that day |

Proven the same way as I1–I9 — nine defects put back in temporary copies (a dead export button,
a Report that stopped being live, a PDF box that did nothing, a folder export missing
experiments, a Journal day without its ticks, an export that throws, CSV cells not quoted…) and
each caught. Each case is **guarded**: one that throws is a finding for that case, not the end of
the run — the first version reported a throwing export as "the harness crashed" and lost the
other seventeen presets with it.

**What it found:** the Report's *Deviations* section only knew Archive protocol parameters.
`deviationsSentence` filtered `expDeviations` to `kind==='param'`, so a changed calculator number
— cells per well, a volume, what people actually change, and the kind `expDeviations` was taught
to find on 2026-08-22 — was in the record PDF's table and absent from the Report, which read as
a run that had gone to plan. It reports every kind now ("Changed from the plan: Seeding, cells /
well 30000 → 61001").

### The tour (`startTour`, Settings → Help)

A guided walk through Labbook the first time it is used, and on demand from **Help** (a new
first tab in Settings, a `?` in the column foot, *More → Help* on the phone). It **moves you
through the app** — Home, Today, Experiments, the project column, an experiment and its Report,
the Journal, Visualize, search, Help — and spotlights the real element on each screen, so it
cannot describe a UI that has moved on. `tourSteps()` builds the list from what exists: the
experiment steps only when there is an experiment to open, the column step not on a phone.

- **Inside dHUB it draws in the shell's document** (`_tourDoc`, same origin) so it can point at
  the rail and the header search; rects of Labbook's own elements are offset by the frame's
  (`_tourOff`). Standalone and the PWA draw in their own.
- **It starts on its own only on an empty notebook** (no experiments, day notes or pages) and
  only once (`lb_tour_done`, set by Done, Skip or Esc). A notebook that already holds something
  — on this device or brought in by the cloud — is somebody who has used Labbook; the tour
  waits under Help. It waits for any open dialog (the one-time backup note) and, in dHUB, for the
  Labbook frame to be on screen.
- It ends where you started: `SEL` is saved at the start and put back.
- The card is measured and clamped by its real edges like every popover; the first placement
  does not animate from the corner; a step with no target dims the page through a 2 px hole,
  because a zero-size box draws no shadow.
- Every harness (`invariants.mjs`, `mobile_sweep.mjs`, `snap_compare.mjs`) sets `lb_tour_done`,
  or a fresh page would open on the tour.

### Tabs that stay put, and screens that cross-fade

Jon: *"el menú settings se mueve demasiado al cambiar de pestañas… transiciones más smooth."*
Measured before touching it: the Settings dialog was centred and sized to each tab, so its top
edge moved **127 → 276 → 305 → 243 px** across four tabs, under the pointer that had just
clicked one.

- **A tabbed dialog is one fixed height** (`.modal.tabbed`, `#settings-back .set-card`) with
  the body scrolling — Labbook's Settings and Recover, dHUB's Settings. Only Settings in the
  shell: the question dialog shares `.set-card` and stays the size of its question. Verified
  steady on every tab at 1440 and 390.
- **A switch replaces only the body** (`settingsTab`/`recoverTab` → `_tabSwap`), which arrives
  with a 4 px rise and fade (`.tab-in`), and **the underline slides** (`_tabInk` / `_hubInk`,
  carried across a rebuild of the bar so it moves instead of reappearing).
- **Changing screen is a real cross-fade.** `renderEditor` already animated the new screen in;
  the old one vanished in one frame. `_screenGhost` places a copy of the leaving screen where it
  was and fades it over the arriving one — a copy, so the render stays synchronous and every
  caller that acts on the new DOM straight after `renderAll()` still finds it. Skipped over
  3,500 nodes and under reduced motion. The columns fade when the workspace changes, the list
  pane slides in when it comes back, and the Designer's body when the step changes (not on a
  redraw of the same step, or every keystroke would strobe).
- **Cells' tabs blinked the whole screen.** Every tab click re-ran the view's fade-in from
  transparent, tab bar included. The view fades on the way in; a switch fades only the frame.

## A finished experiment reads as a record (2026-09-24)

Jon: opening a completed experiment showed nothing — every past day whose steps were all ticked
starts folded in Steps (`_benchFolded`), and on a finished run that is every day. **A closed
experiment (`expClosed`: done · paused · abandoned · archived) folds nothing by default** and its
ticked rows are not dimmed (`.bench.closed`), so the text, the recipe and the numbers read at
full strength. *Fold done days* and a click on a day header still fold, per session. The
Observations box lost its 180px floor (52px now): it is one line about the run as a whole, and
the empty space pushed Results and Files a screen down.

## The Designer, broken on purpose (2026-09-25)

Jon: *"analiza el experiment designer entero, qué parámetros faltan… Break the designer, find
everything that is wrong with it, fix it and add features"* — plus NanoBRET is not always
target engagement, and a filter for default presets against the lab's own.

**NanoBRET is "NanoBRET".** The type, the preset's name and title and the auto-title of every
run said *target engagement*; most of what this lab runs is a PPI. `_nbRename()` runs at the
end of `seedPresets` (boot and adoption), per record and not flag-gated, and only touches a
title that is exactly the old auto-title. A tracer-displacement preset is future work. The
`SYNTH_PARTS.__nanobret` protocol part really is TE and keeps its name.

**Bugs found by trying to break it, each reproduced first:**

- **Typing in "Days after" lost the caret after one digit** — `oninput` called `dsDraw()`,
  so *12* was stored as *1*. `dsSetGap` stores and updates only the day label; the screen
  redraws on `change`.
- **Deleting a design broke every run made from it.** `presetBase(key)` of a key that no longer
  exists is the key itself, so the Custom schema came back: Edit setup offered 1 question of
  11. Runs now carry `e.setupAdd`/`e.setupHide`, and **`expSetupFields(e,su)`** is the one reader
  (setup editor, setup chips) — record first, preset only for older runs.
- **Prose edited in the Designer on a preset-based run was put back to the preset's on the next
  load.** The wording pass's proof was "unchanged since created" — true of your own words too.
  A module remembers what the preset gave it (`m.orig`); anything else is stamped
  **`b.tpl.own`** at creation (`dsTemplate(D, forRun)` → `tplOwn` → `applySetupToBlocks` sets
  `tpl.own`), and `_expBlockUntouched` returns false for it — so neither the pass nor the
  "update from the preset" banner can touch it.
- **The NanoBRET preset's 6-well transfection seeding was rescaled to the 384-well read** —
  800,000 cells in 2 mL became 4,700 in 30 µL under prose that still said "6-well", in the
  quick window too. A calculator may now be **on its own plate** (`calc.fmtOwn`,
  `_calcOwnPlate`): the run's format, cells/well and # plates do not apply to it, the cell line
  still does. `applySetupToBlocks`, `dsSyncMod`, `dsOwned`, `dsPlateAt` and `_setupTag` honour
  it; the Designer has an *its own plate* tick on any calculator with a format. The NB seed
  carries it (`_presetV17`).
- **Compounds typed in the Designer went into the quick window's state.** `_setupListUpd` and
  `_setupApplyPlates` knew two panels (`esSetupUpd` → `ES_SETUP`, anything else → `NM_SETUP`);
  `_setupSrc(upd)` knows the third.
- **The curated presets were offered a Delete that did nothing** — `seedPresets` put them back
  on the next load — and *Save* wrote over them. **`isDefaultPreset(k)`** is the ten types
  *and* `EXTRA_PRESET_SEED`; `isBuiltinPreset` still means only the ten (`allPresetKeys` sorts
  by it). A default can be opened, duplicated, exported and restored, never deleted; saving one
  from the Designer makes a copy "(my version)".
- A Blank + plate map had no way to choose the format; the layout was only ever the preset's;
  **Cancel discarded everything unasked**, and nothing in the wizard could be undone.

**Default presets / your presets.** `presetFilterChips` (All · Default presets · Your presets,
with counts, remembered in `lb_ds_pf`) and `presetFilterKeys(f,q)` — yours first — on the
Designer surface and on the wizard's first screen, which now lists the designs inline with a
search (`dsDesignsHtml`) instead of one underlined link. The quick window's select is two
`optgroup`s (`presetOptionsHtml`), defaults first because the modal opens on the first option.
Badges: *default*, *yours*, *edited* (a default whose `_seedSig` no longer matches —
`presetEdited`), with **Restore default** (`restoreDefaultPreset`). Designs **export** as
`.design.json` and **import** (`dgExport`/`dgImport`), arriving as the lab's own.

**What the Designer can say now that it could not:**

- **Your own parameters** (`dsParamDlg`): a question the type does not ask — number, text,
  yes/no, choice, list — stored in `D.setupAdd` as `{f,t,lbl,d,opts,custom:true}`, asked like
  any other, quoted in any step as `{{key}}` (the **Insert a parameter** chips put it at the
  caret, in the prose or the Methods box), carried by a saved design, onto the record, and into
  Edit setup. They survive a change of type. They do not print on the setup line
  (`_pubSetupText` walks the type's fields) — the step's own sentence is where they belong.
- **Start the map from** any `PLATE_LAYOUTS` entry for the format, the automatic choice, or
  **an empty plate** (`'none'`). `experimentPlateFor` takes the choice as its 7th argument;
  `dsPlateFor(D,ps)` is the one call the review and the invariants use.
- **Modules:** duplicate, **repeat N times every X days** (the step after keeps its day while
  there is room), **add modules after this one**, a jump strip between modules while
  configuring, and a ⋯ / right-click menu.
- **Your modules** — a configured step saved for reuse (`LB.data.myModules`, a new synced
  root in `_CLOUD_ROOTS`), first in the module picker and in a running experiment's step
  palette; `libSpec` falls back to `_myModSpec`.
- **Biological replicates at creation** — N runs, X days apart, linked by `repGroup` exactly as
  *Repeat as replicate* links them (`buildExperimentFrom` gained `onMade`), codes via
  `_replicateCode`.
- **A run made only of Archive protocols** is allowed (Next goes straight to the review).
- **Undo and drafts.** `DS_UNDO` (snapshots before every destructive action, ⌘Z in the wizard,
  an Undo button) — kept outside `DS` so I6 does not read it as a draft key. Any change makes
  the draft dirty and it is saved to `localStorage['lb_ds_draft']` (this device only, 14 days);
  Cancel/Escape ask *keep designing · close and keep the draft · discard*; *Resume* is on the
  Designer surface and in the New-experiment chooser.

**Invariants I10–I16**, each proven by putting its bug back: I10 your words survive the wording
pass · I11 a run's questions outlive its design · I12 typing keeps the focus (722 boxes) · I13
defaults cannot be deleted and a copy is yours · I14 own parameters round-trip · I15 N
replicates are N linked runs · I16 every parameter typed is the value stored. I1 gained an
...
## One door per function, and a Designer that stays connected (2026-09-26)

Jon: *"hay varios sistemas de interacción redundantes… una sola interacción clara por función."*
The other apps turned out mostly clean inside themselves (their gears are hidden in dHUB); the
duplication was where the shell and Labbook meet. Removed: dHUB's **Lab** button (it opened
Settings on its Lab tab), the Settings row that collapsed the rail (the rail has its own fold),
and Labbook's second Settings, Help and theme buttons in the column foot (the top bar's gear is
...
- **The attachment sweep has run and the backfill is done** (checked 2026-09-22 against
  `Backup/labbook-*.json`, read-only, mirroring `attNeedingUpload()`): 13–15 Sep no
  attachments at all; 16 Sep 1, with a cloud url; **17 Sep 3 referenced, 1 needing upload;
  18 Sep 3 referenced, 0 needing upload** — the sweep uploaded it. Nothing outstanding on that
  machine. Note the sweep can only ever run *in the browser that holds the bytes*: `_attGet`
  reads the local `lb_att` store, so a file added on the phone uploads from the phone. It runs
  itself 9 s after boot and on demand at Recover → Backup file → *Upload them now*.
- **Lumina** — see *Lumina is Echo's counterpart, not a mode of it*. Not a merge; the work is
  Send to Labbook and the out-of-range flag.
...
- **Fold in (Phase 2)**: Beacon → an Echo assay type; one plate engine (Labbook's, plus
  Blueprint's `pdParseValues`);
