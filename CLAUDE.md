...
**Suite-wide survey (not fixed, scope was Protocols-only per Jon's explicit priority):** grepped all 13 apps' `header{...}` rules — **every single one uses the same fixed `height:58px` pattern** Protocols had (Cuppa uses `height:54px`), so the identical overlap-bug *class* is latent everywhere; whether it actually fires depends on whether that app's header `<p>` subtitle is long enough to wrap at narrow widths. Most apps have no subtitle `<p>` in the header at all (Arc, Beacon, Helix, Lumina, Plate_Designer/Blueprint, Protein Tools, Ribbon, Spectra — likely near-zero risk). Two stand out as likely actually broken the same way: **Iceberg** (`Cryostorage/cryostorage.html`, 86-char subtitle — longer than Protocols' 56-char one that did break) and **Echo** (`Labcyte_Echo/labcyte_echo.html`, 72 chars). LDI (36 chars) and Cuppa (16 chars) are shorter and lower-risk but not verified clean. **None of these 12 other apps were touched this session** — flagged for Jon to decide whether to greenlight a follow-up pass, since the fix pattern (found and validated on Protocols) is now known and would translate directly: `height`→`min-height` on the header rule, plus a `@media(max-width:480px){header p{display:none}}` rule, in each app file.
