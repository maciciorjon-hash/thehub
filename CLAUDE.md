...
**EXPERIMENTAL, on branch `feature/protocols-suite` — NOT merged to main, NOT yet a version bump:** Jon wants LabMate's protocol/calculator content pulled out into its own polished app, one card per assay/experiment type, each with **Protocol / Calculate / Output** tabs (matching Echo's tab-bar convention), explicitly asked as a revertible experiment ("si no me gusta quiero revertir todo"). Ran a full content audit of `Labmate/labmate.html` first (10,976 lines) via a research subagent before building anything — findings: ~29 protocol/calculator tools across 8 sections (Calculators, Mol Biology, CRISPR, Cell Biology, Proteomics, Genomics, Struct Bio, Biophysics), roughly half genuine calculators and half pure reference text; Cell Lines and the 6-chapter "Pip's Story" easter egg are separate content that stays in LabMate; LabMate already has an internal `.has-calc`/`mode-calc` split between narrative and calculator markup plus a working `renderExportPanel()` copy-to-clipboard system — both mapped almost directly onto the new tab design, which is why porting math/markup was straightforward rather than a rewrite. Also found real dead code (Buffers calc, Reference Tables, PROTAC/Dmax normalizer, an Echo Acoustic Dispenser calculator, an NGL PDB viewer, a Gel Diagram stub — all ~zero HTML container, i.e. unreachable) and two functional-but-nav-orphaned sections (Lab Timers, Experiment Planner, reachable only via cross-app deep link) — neither touched, out of scope.

Built new **`Protocols/protocols.html`** (app id `protocols`, accent `#a56983` — the last unused Nature-palette color) as a working prototype, not a stub: home view with search + category-filter pills + a card grid (data-driven from a `PROTOCOLS` array), detail view per protocol with the 3-tab pattern. Migrated **3 protocols** end-to-end as representative proof points rather than all 29, specifically chosen to stress-test every pattern variant before investing in the mechanical rest: **Simple Dilution (C1V1)** (simplest calculator), **Gibson Assembly** (complex multi-step calculator — PCR sub-tabs, DpnI reminder, fragment-adder, already had an export panel in LabMate worth directly reusing), **Western Blot** (pure reference, proves the "no Calculate tab" variant, Output tab falls back to "copy the Protocol tab's text" + PDF instead of a structured export). Output tab supports PDF (jsPDF, same CDN convention as Echo's `cvDownloadPDF`/`generateCurvePDFs`), copy-to-clipboard step panels (ported `renderExportPanel`/`copyExportStep`/`copyExportAll`/`clipboardWrite`/`fallbackCopy` verbatim from LabMate — fully generic, no changes needed), and CSV (plain-`Blob`-download, Excel-openable — deliberately skipped bundling full SheetJS for this prototype, since Echo/Dora load it lazily from CDN only on actual export and that's a cheaper, equally-safe pattern to replicate later if full multi-sheet `.xlsx` fidelity is ever wanted). LabMate's "Pip Tip" callouts were renamed to a neutral "Tip" badge with no mascot branding, matching Jon's "todo dHUB se tiene que sentir unificado" ask — Pip is LabMate's own character, not global dHUB branding. Wired into the shell exactly like any other app: `embed.py` APPS list, `APP_B64_NEW`/`APP_INFO`/`ALL_APP_IDS`/`APP_UNLOCK_WORDS` (word `benchwork`)/`PACKAGES` (added to the existing **Molecular Biology** group alongside Helix/Protein Tools/LabMate)/`HUB_SEARCH_INDEX`/home card/`app-view` iframe — no `embedStyle` special-case needed, it uses a plain `<header>` so it falls through to the shared default (`header{display:none!important}`) like most apps. Verified via Playwright at every layer: standalone file (search/filter, both calculators computing correctly — 10 µM→1 µM in 100 µL gives 10 µL stock + 90 µL diluent, Gibson's full 3-step pipetting table, PDF/CSV/copy buttons all firing, dark mode), and embedded inside the rebuilt `dHUB.html` (breadcrumb, icon, header-hiding all correct, zero console errors). **Known transitional overlap, not yet cleaned up:** the 3 migrated protocols still also exist untouched in LabMate (nothing removed from LabMate yet) — `HUB_SEARCH_INDEX` now has near-duplicate entries (e.g. "Simple Dilution" under LabMate and "Simple Dilution (C1V1)" under Protocols) until Jon approves and a follow-up pass removes the LabMate originals. **Next steps, only after Jon reviews and approves the direction:** mechanically migrate the remaining ~26 tools (now a repeatable pattern, not a design problem), remove migrated content from LabMate, dedupe the search index, then treat it as a normal same-day version bump + changelog entry + merge to `main` + push. If Jon doesn't like the direction: `git checkout main` — nothing on `main` was touched, the whole thing was built on this one branch.
